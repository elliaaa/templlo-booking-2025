...
public class ReservationConfirmProducer {
    ...
    public void send(String reservationConfirmedTopic, ReservationConfirmMessage message) throws JsonProcessingException {

        String reservationConfirmKey = "reservationId:" + message.reservationId();

        //ReservationConfirmMessage storedMessage = objectMapper.readValue((String) redisTemplate.opsForHash().get(reservationConfirmKey, "body"), ReservationConfirmMessage.class);

        String body = (String) redisTemplate.opsForHash().get(reservationConfirmKey, "body");

        ReservationConfirmMessage storedMessage = null;

        if (body != null) {
            // body 값을 ReservationConfirmMessage로 변환
            storedMessage = objectMapper.readValue(body, ReservationConfirmMessage.class);
        }

        if (storedMessage == null || !storedMessage.equals(message)) {
            log.warn("Message validation failed for reservationId: {}. Skipping send.", message.reservationId());
            return;
        }
